...
from .slide_utils import read_slide, read_region_from_slide, read_full_slide_by_level, get_slide_and_mask_paths, \
    get_connected_regions_from_tumor_slides, LEVEL_BASE, HIGHEST_LEVEL

"""
Only use for reference; provide useful functions for slide extractions 
"""
...
def crop_slide_as_patches(slide_path,
    ...
    thumbnail = read_full_slide_by_level(slide_path, HIGHEST_LEVEL).convert("HSV")  # use HIGHEST_LEVEL as thumbnail
    ...
    if strategy == "default" or mask is None:
        ...
        zoom_factor = LEVEL_BASE ** (HIGHEST_LEVEL - level)  # todo: double check the mapping here
...
def crop_all_slides(slide_paths, 
                    mask_paths,
                    save_dir, level,
                    pos_preserve_weight=1.0,
                    strategy="default",
                    workers=4):
    save_dirs = [Path(save_dir) / f"level_{level}/{str(slide_path.name).replace('.tif', '')}" 
...
def get_zoom_out_context(slide_path,
    ...
    factor = LEVEL_BASE ** (output_level - input_level)
...
def reconstruct_from_patch(meta_info_df, level_info_df, level, slide_id, resize_factor=None):
    if resize_factor is None:
        resize_factor = 10 * 4 ** (int(HIGHEST_LEVEL / level))

    size = meta_info_df.iloc[slide_id][f"Level_{level}"][1]
    img = Image.new('RGB', size)

    slide_name = meta_info_df.iloc[slide_id].id
    ...
    tumor_mask = Image.new("RGBA", (SIZE, SIZE), (173, 255, 47, 30))
